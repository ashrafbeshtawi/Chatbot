...
public class SpoonfulSearchParser {

	//Input name of recipe to look up
	private final Document htmlToParse;

	private static final Logger LOGGER = LoggerFactory.getLogger(SpoonfulSearchParser.class);
	
	public SpoonfulSearchParser(final Document htmlToParse){
		this.htmlToParse = htmlToParse;
	}
	
	/*
	 * Get the links to all of the results on the first page
	 * @return
	 * @throws IOException
	 */
	public List<String> getLinks() {
		...
		try {
			final Elements articles = this.htmlToParse.select(".wrapper").select("#content").select("article");
