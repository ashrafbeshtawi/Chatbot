...
import zone.bot.vici.pattern.model.ManyNode;
...
class ManyNodeMatcher implements PatternNodeMatcher<ManyNode> {

	@Nonnull
	private final ManyNode node;
	@Nonnull
	private final PatternNodeMatcher<?> child;

	ManyNodeMatcher(@Nonnull final ManyNode node, @Nonnull final PatternNodeMatcher<?> child) {
		this.node = node;
		this.child = child;
	}
	...
	@Override
	public ManyNode getPatternNode() {
		return this.node;
	}
}
